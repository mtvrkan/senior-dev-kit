...
>
> **How this relates to the rest of the kit:** This bootstrap deliberately generates a **lean 7-agent roster** (architect, security-reviewer, implementer, test-author, reviewer, debugger, researcher) sized for a brand-new project — it does **not** install the kit's prebuilt agents, skills, or commands. The generated team is a from-scratch template tailored to your stack; the prebuilt kit is a broader, maintained catalogue. If you later want the full kit in the same project, install it per README.md's Install section on top — the two coexist, and the kit's installer backs up anything it would overwrite.
>
> **Every "hook" mentioned below (1.2, 1.6, 2, and the `settings.json` template in 1.5) is generated INTO the new project you're bootstrapping** — none of them is a feature of the senior-dev-kit repo itself. The kit registers exactly one hook of its own, and only in a plugin install: a `SessionStart` hook that injects the global protocol and writes nothing (see `SECURITY.md`). Don't read the hooks below as claims about this kit's own enforcement.
