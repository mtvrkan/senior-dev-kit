...
>
> **How this relates to the rest of the kit:** This bootstrap deliberately generates a **lean 7-agent roster** (architect, security-reviewer, implementer, test-author, reviewer, debugger, researcher) sized for a brand-new project — it does **not** install the kit's 7 prebuilt agents, 24 skills, or 3 commands. The generated team is a from-scratch template tailored to your stack; the prebuilt kit is a broader, maintained catalogue. If you later want the full kit in the same project, copy the kit's files per README.md's Kurulum section on top — the two coexist; back up any `.claude/` files you'd overwrite first.
>
