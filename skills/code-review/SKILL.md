...
Do not use for: a dedicated, exhaustive security audit (`security-review`) — items (5) and (7) below are an inline sanity check on the diff, not a substitute for one; dependency CVE checks (`security-scan`); pre-release gating (`release-gate`); or a deep performance investigation (`performance-check`) — item (10) below is a quick flag, not a profiling pass.
