...
description: Use to generate or refresh a codebase architecture overview — directory map, data flow, key component responsibilities — so future research reuses this instead of rediscovering the codebase file by file.
allowed-tools: Read, Grep, Glob, Bash, Write
when_to_use: Use when starting work on an unfamiliar codebase, when no architecture overview exists yet, or when the project has changed structurally since the last one was written.
