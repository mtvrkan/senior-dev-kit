---
name: security-scan
description: Use for dependency audits (CVEs, outdated majors, paid licenses, bloat), secret scans, SAST, container/filesystem scans, and release security checks. Do not use for low-risk UI-only tasks.
when_to_use: Use when dependency audit, secret scan, SAST, or container security review is needed. Auto-trigger on dep add/update, auth/payment/DB/API/secrets/CI/release changes, or explicit user request.
