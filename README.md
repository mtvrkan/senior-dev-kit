...
Currently: 346/346 tests passing (55 suites). `routing-eval` pins 26 realistic requests against
...
**What that does and does not prove.** Be clear-eyed about it: those 346 tests are *internal
...
The one behavioral measurement is the live arm of `routing-eval`, and it is opt-in because it
costs API credits:
...
It runs an A/B over the 26 golden prompts — two CLI calls each, 52 in total: a **control** arm
...
the treatment arm failing to beat control (`ROUTING.md` is loaded into context every session, so
it has to earn that); or any prompt that plain descriptions route correctly and `ROUTING.md`
breaks — checked separately from the net lift, which would otherwise let breakage cancel against
improvement.

**The measured result, 2026-08-11:** control 22/26 (85%), treatment 26/26 (100%) — a lift of
+15.4 points, with no prompt that the descriptions alone route correctly broken by the table. The
four it moved were the ones where a guard-area noun has to outrank the verb in the sentence
(`fix CSS in the login form` → `security-guard`, not `ui-fixer`). Those integers live in
`eval/golden-prompts.json`, and `check-consistency` fails if this paragraph states a score that
file does not. One run on one model version; re-run it yourself if you want it fresh.

That covers routing, one component of several. There is no A/B behind the rules, the skills, or
the presets, and no before/after on code quality — those rest on judgment, not measurement. If
you are deciding whether to adopt this kit, weigh it accordingly.
