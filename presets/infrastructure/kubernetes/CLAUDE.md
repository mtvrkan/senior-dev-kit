...
`rules/600-devops.md` co-loads for `k8s/`, `manifests/`, `deploy/`, `charts/*/templates/` and
`*.k8s.*`, and owns the pipeline side: SHA-pinned Actions, OIDC, image scanning, SBOM, and the
security checklist a manifest is graded against. This preset is the workload side — what a
Deployment must declare before it ships, and the runtime failure each declaration prevents.
