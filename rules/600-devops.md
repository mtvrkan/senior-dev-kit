...
paths:
  ...
  - "**/*.k8s.*"
  - "**/helm/**"
  # `kubernetes/` and `helm/` are the least common of the layouts people actually use: a chart
  # lives in `charts/` by Helm's own convention, and manifests sit in `k8s/`, `manifests/` or
  # `deploy/`. Without these four the kubernetes preset shipped against a rule that never loaded
  # for the files it describes. Pinned in `scripts/rule-globs.test.ts`.
  # `k8s/` is unambiguous; the other three are extension-scoped on purpose, because a frontend
  # `components/charts/BarChart.tsx` and a `deploy/` full of app code must not pull in Docker
  # and CI rules. Helm's own layout (`charts/<name>/templates/`) is what the chart glob targets.
  - "**/k8s/**"
  - "**/charts/*/templates/**"
  - "**/charts/*/{Chart,values}*.{yaml,yml}"
  - "**/manifests/**/*.{yaml,yml}"
  - "**/deploy/**/*.{yaml,yml}"
...
Tools: Checkov (Terraform + K8s + ARM) — preferred, pin to a specific released version (e.g. `checkov==3.3.x`), never `latest`.
...
- [ ] Resource requests (`requests.cpu` + `requests.memory`) on every container — without them the
      scheduler guesses and the pod is first to be evicted
- [ ] `limits.memory` on every container. A CPU limit throttles rather than kills, so it is a
      deliberate choice, not a checklist item — never set it equal to the request
