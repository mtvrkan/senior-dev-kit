...
# the name and Copy-Item -Force would clobber (or nest into) the earlier
# backup. Append a counter until the name is free.
function Get-UniqueBackupPath($base) {
    $candidate = $base
    $n = 1
...
function Backup-Dir($dest) {
    if (Test-Path $dest) {
        ...
        if ($hasContent) {
            $timestamp = Get-Date -Format "yyyyMMddHHmmss"
            $backup = Get-UniqueBackupPath "$dest.bak.$timestamp"
...
if ($script:ClaudeMdBackup) {
    Warn "Your previous CLAUDE.md was replaced — the old copy is at: $script:ClaudeMdBackup"
}
Write-Host ""
