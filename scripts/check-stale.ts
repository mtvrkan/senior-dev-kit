...
// Each maintenance file opens with a count claim ("...the 34 skills in `skills/`").
// README counts are already verified against disk; these headers drifted silently
// (33 vs 34 skills, 12 vs 13 commands) because nothing checked them — so check them.
// The claim is the first integer in the intro (before the first table) whose sentence
// names the content type — anchoring to the noun so unrelated numbers (a date in a
// note, a "12 months" cadence bullet) can't be mistaken for the count. "commands"
// appears as "slash-command definitions" and "agents" as "subagent definitions", so
// the singular stem is matched. A file whose intro drops the claim skips this check.
function checkHeaderCount(fileContent: string, fileLabel: string, dirLabel: string, actual: number): void {
  const intro = fileContent.split('\n|')[0]
  const stem = dirLabel.replace(/s$/, '')
  const m = intro.match(new RegExp(`\\b(\\d+)\\b[^.\\n]{0,60}?${stem}`, 'i'))
  if (!m) return
  const claim = parseInt(m[1], 10)
  if (claim !== actual) {
    headerCountMismatches.push(`${fileLabel}: header claims ${claim} ${dirLabel}, found ${actual} on disk`)
  }
}
...
const headerCountMismatches: string[] = []
let presetsChecked = 0
const trackedPresets = new Set<string>()

flagMalformedReviewRows(content, basename(MAINTENANCE_FILE))
if (existsSync(PRESETS_DIR)) {
  checkHeaderCount(content, basename(MAINTENANCE_FILE), 'presets', findPresetDirs(PRESETS_DIR).length)
}
...
function checkFlatMaintenance(maintenanceFile: string, dirLabel: string, entries: string[]): number {
  ...
  checkHeaderCount(fileContent, fileLabel, dirLabel, entries.length)
...
if (headerCountMismatches.length > 0) {
  console.error(`\n✗ ${headerCountMismatches.length} maintenance file header count(s) out of sync with disk:\n`)
  for (const m of headerCountMismatches) {
    console.error(`  ✗ ${m}`)
  }
  console.error(`\nUpdate the count in the file's intro paragraph so it matches disk.`)
  exitCode = 1
}
