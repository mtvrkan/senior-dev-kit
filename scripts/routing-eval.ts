...
// naming several agents is ambiguous and scores as a miss.
function extractRoutedAgent(answer: string): string | null {
...
if (process.env.RUN_ROUTING_EVAL === '1' && exitCode === 0) {
  ...
  for (const entry of golden.prompts) {
    ...
    const got = extractRoutedAgent(answer) ?? answer.slice(0, 40)
    if (got === entry.expect) {
      passed++
