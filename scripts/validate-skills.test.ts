...
import { globToRegExp as bashGlobToRegExp } from './deny-cost.ts'
...
describe('validate-skills integration', () => {
  ...
  test('settings-template.json Read-denies every common JS/package-manager lockfile', () => {
    ...
    function globToRegExp(pattern: string): RegExp {
      const GLOBSTAR_TOKEN = 'GLOBSTAR_TOKEN'
      const escaped = pattern
        .replace(/[.+^${}()|[\]\\]/g, '\\$&')
        .split('**').join(GLOBSTAR_TOKEN)
        .replace(/\*/g, '[^/]*')
        .split(GLOBSTAR_TOKEN).join('.*')
    ...
    for (const lockfile of lockfiles) {
      ...
      const covered = readPatterns.some(p => globToRegExp(p).test(samplePath))
  ...
  test('settings-template.json Read-denies id_rsa/id_ed25519 filename variants, not just the exact name', () => {
    ...
    for (const variant of ['./project/id_rsa.bak', './project/id_rsa_prod', './backup/id_ed25519_work']) {
      const covered = readPatterns.some(p => globToRegExp(p).test(variant))
  ...
  test('every secret Read(...) pattern in settings-template.json has a base64/Get-Content companion (derived from the deny list, not a hardcoded copy of it)', () => {
    ...
    // Read-pattern-shape glob→regex (`**` crosses `/`, single `*` doesn't) —
    // duplicated from the lockfile/id_rsa tests above rather than extracted,
    // matching this file's existing per-test convention; deliberately NOT
    // `bashGlobToRegExp` (imported from deny-cost.ts), whose `*` matches
    // across `/` because it's built for Bash/PowerShell command tails, not
    // filesystem paths — using it here would silently accept a broken sample.
    function readGlobToRegExp(pattern: string): RegExp {
      const GLOBSTAR_TOKEN = 'GLOBSTAR_TOKEN'
      const escaped = pattern
        .replace(/[.+^${}()|[\]\\]/g, '\\$&')
        .split('**').join(GLOBSTAR_TOKEN)
    ...
    for (const pattern of secretPatterns) {
      ...
      assert.ok(
        readGlobToRegExp(pattern).test(sample),
